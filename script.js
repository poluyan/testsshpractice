window.onload = function()
{
    ...
    //const lines = reader.readAsText(filem);
      ...
      Array.from(asymIdSet).sort().forEach(item => {				
		...
		axios.post(`https://cloudkmc.vercel.app/api/formd?name=${dothis}&valueTaskID=${valueTaskID}&valueEmail=${valueEmail}&valueChains=${valueChains}&valueRCSB=${valueRCSB}&userId=${userId}`, data, config ).then(function(res)
		{
			data.delete("filemon");
		})
		.catch(function(err)
  ...
  return fetch(url)
	...
	{		
      ...
      else if(pdbId.length > 0 && pdbId.length < 4)
      {
				...
				const dothis = 0;
				axios.get(`https://cloudkmc.vercel.app/api/formd?name=${dothis}&valueTaskID=${valueTaskID}&valueEmail=${valueEmail}&valueChains=${valueChains}&valueRCSB=${valueRCSB}&userId=${userId}`)
				.then(function(res)
				{
					document.getElementById("showpdb").disabled = true;
				})
